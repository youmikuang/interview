...
func (r *HttpRepo) Query(ctx context.Context, ip domain.IPAddress, channel string) (*domain.Location, error) {
	...
	if !ok {
		return res, fmt.Errorf("invalid channel: %s", channel)
	}
	...
	if e != nil {
		return nil, fmt.Errorf("api content error, can't be parsed: %s", e)
	}
	loc = handleTransformData(result, channel, loc)
	return &loc, nil
}

func handleTransformData(result map[string]interface{}, channel string, loc domain.Location) domain.Location {
	loc.Province = "未知"
	loc.City = "未知"
